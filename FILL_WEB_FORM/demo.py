...
# We eliminate the possibility of having another file
# with the same name with the wrong information.
wb.Sheets("Hide").visible = True
ws = wb.Sheets("Hide")
if ws.cells(1, 2).value != "Demo":
    msgbox(msg="This file must be called by the WorkBook: Demo.xlsm",
           title="Error")
    sys.exit()
# xlSheetVeryHidden
wb.Sheets("Hide").visible = 2
